...
function draw() {
  ...
  if (inMove) {
    ...
    if (moveClock >= moveSpeed) {// move update
      ...
      handIndex++;
      print("handIndex is at " + handIndex);
      ...
      if (handCount > 0) {//if you still have pebbles in hand
        ...
        if (!pots[handIndex].isBig) {// if you end on a normal pot
          ...
          if (pots[handIndex].count > 1) {

            handCount = pots[handIndex].count;
            pots[handIndex].count = 0;
          ...
          print("Ended on a a big pot with handIndex at " + handIndex);
